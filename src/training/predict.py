...
def predict_stock_list(stock_list: list, date=None) -> pd.DataFrame:
    ...
    with get_db_session() as db:
        report_df = get_predict_report_by_date(db, date)
...
def process_predict(report_date=None, sync_all=True, import_2_aws=True):
    if report_date is None:
        with get_db_session() as db:
            report_date = get_last_index_daily_date(db)
    ...
    df = predict_stock_list(stock_code_list, report_date)
    with get_db_session() as db:
