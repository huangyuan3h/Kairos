...
from upload2aws.upload_classify_to_dynamodb import import_classify_2_aws_process


def predict_stock(stock_code: str, predictor: ModelPredictorClassify, date: datetime.date, data_version="v1"):
    if date is None:
        return None
    end_day = date
    # 计算100天前日期
...
def predict_stock_list(stock_list: list, date_object: datetime.datetime = None,
    ...
    predictor = ModelPredictorClassify(version)
    ...
    with get_db_session() as db:
        report_df = get_predict_classify_report_by_date(db, date_object.strftime("%Y%m%d"))
...
def process_predict_classify(report_date=None, sync_all=True, import_2_aws=True, version="simple_lstm_v1_2"):
    ...
    if import_2_aws:
        ...
        import_classify_2_aws_process(report=df)
